...
class GenericScripting {
  constructor(sandboxBundleSrc) {
    this._ready = new Promise((resolve, reject) => {
      const sandbox = import(/* @vite-ignore */ /*webpackIgnore: true*/ sandboxBundleSrc)
      sandbox
        .then((pdfjsSandbox) => {
...
class NewAltTextManager {
  ...
  async editAltText(uiManager, editor, firstTime) {
    ...
    let canvas
    if (mlManager) {
      ;({ canvas, imageData: this.#imageData } = editor.copyCanvas(AI_MAX_IMAGE_DIMENSION, true))
      if (hasAI) {
        this.#toggleGuessAltText(await isAltTextEnabledPromise, true)
      }
    } else {
      ;({ canvas } = editor.copyCanvas(AI_MAX_IMAGE_DIMENSION, false))
    }
...
const PDFViewerApplication = {
  ...
  async _parseHashParams() {
    ...
    const loadPDFBug = async () => {
      ...
      const { PDFBug } = await import(/* @vite-ignore */ /*webpackIgnore: true*/ AppOptions.get("debuggerSrc"))
      this._PDFBug = PDFBug
    }
    if (params.get("disableworker") === "true") {
      try {
        GlobalWorkerOptions.workerSrc ||= AppOptions.get("workerSrc")
        await import(/* @vite-ignore */ /*webpackIgnore: true*/ PDFWorker.workerSrc)
      } catch (ex) {
